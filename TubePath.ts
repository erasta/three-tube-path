...
interface TubePathParameters {
	path: Curve<Vector3>;
	uMappingFrames: number[] | undefined;
	radius: number;
...
export class TubePath extends BufferGeometry {
	...
	declare type: string;
	parameters: TubePathParameters;
	tangents: Vector3[];
	normals: Vector3[];
	binormals: Vector3[];

	constructor(
		path: Curve<Vector3> = new CatmullRomCurve3([new Vector3(-1, -1, 0), new Vector3(-1, 1, 0), new Vector3(1, 1, 0)]),
		uMappingFrames?: number[],
		radius = 0.3,
		radialSegments = 8,
		closed = false,
	) {
		...
		if (!uMappingFrames) {
			uMappingFrames = TubePath.pathToUMapping(path as CatmullRomCurve3);
		}
	...
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	static fromJSON(data: any): TubePath {

		// This only works for built-in curves (e.g. CatmullRomCurve3).
		// User defined curves or instances of CurvePath will not be deserialized.
		const curveTypes: Record<string, new () => Curve<Vector3>> = {
			CatmullRomCurve3,
		};
		const CurveClass = curveTypes[data.path.type];
		if (!CurveClass) {
			throw new Error(`Unknown curve type for deserialization: ${data.path.type}`);
		}
		return new TubePath(
			new CurveClass().fromJSON(data.path),
			data.uMappingFrames,
			data.radius,
			data.radialSegments,
			data.closed
	...
	static pathToUMapping(
		path: CatmullRomCurve3 = new CatmullRomCurve3([new Vector3(-1, -1, 0), new Vector3(-1, 1, 0), new Vector3(1, 1, 0)]),
		elbowSegmentNum = 2,
		...
		const uMappingFrames = lengths.map(l => l / lengths[lengths.length - 1]);
